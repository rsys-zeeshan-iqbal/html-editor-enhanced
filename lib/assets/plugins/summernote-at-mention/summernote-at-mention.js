  ...
  $.extend($.summernote.plugins, {
    summernoteAtMention: function(context) {
      ...
      this.handleSelection = () => {
        ...
        const currentText = anchorNode.nodeValue;
        const { word, absoluteIndex } = this.findWordAndIndices(
          currentText || "",
          selection.anchorOffset
        );
        ...
        const beforeText = currentText.slice(0, absoluteIndex);
        const afterText = currentText.slice(absoluteIndex + word.length);

        const beforeNode = document.createTextNode(beforeText);
        const afterNode = document.createTextNode(" " + afterText);

        const mentionSpan = document.createElement("span");
        mentionSpan.style.color = MENTION_COLOR;
        mentionSpan.style.fontWeight = "600";
        mentionSpan.textContent = "@" + newWord;
        mentionSpan.contentEditable = "false";

        parentEl.insertBefore(beforeNode, anchorNode);
        parentEl.insertBefore(mentionSpan, anchorNode);
        parentEl.insertBefore(afterNode, anchorNode);
      ...
      this.renderAutocompleteContainer = () => {
        ...
        const s = this.autocompleteContainer.style;
        s.position = "absolute";
        s.top = String(this.autocompleteAnchor.top) + "px";
        s.left = "0px";
        s.right = "0px";
        s.backgroundColor = "#ffffff";
        s.borderRadius = "8px";
        s.boxShadow = "0 2px 12px rgba(0,0,0,0.15)";
      ...
      this.renderAutocomplete = () => {
        ...
        autocompleteContent.style.padding = "4px 0";
        ...
        this.suggestions.forEach((suggestion, idx) => {
          ...
          suggestionDiv.textContent = trimmed;
          suggestionDiv.style.padding = "10px 14px";
          suggestionDiv.style.fontSize = "14px";
          suggestionDiv.style.fontFamily = "-apple-system, BlinkMacSystemFont, sans-serif";
          suggestionDiv.style.cursor = "pointer";
          suggestionDiv.style.lineHeight = "1.4";
          suggestionDiv.style.color = "#212121";
          suggestionDiv.style.maxLines = "2";
          suggestionDiv.style.overflow = "hidden";
          suggestionDiv.style.textOverflow = "ellipsis";
          suggestionDiv.style.whiteSpace = "nowrap";

          if (idx < this.suggestions.length - 1) {
            suggestionDiv.style.borderBottom = "1px solid #e8e8e8";
          }

          if (this.selectedIndex === idx) {
            suggestionDiv.style.backgroundColor = "#E8F5E9";
            suggestionDiv.style.color = MENTION_COLOR;
            suggestionDiv.style.fontWeight = "600";
          }
      ...
      this.hideAutocomplete = () => {
        if (!this.showingAutocomplete) return;
      ...
      this.setAutocompleteAnchor = (atTextIndex, indexAnchor) => {
        ...
        this.autocompleteAnchor = {
          top: anchorBoundingRect.top + anchorBoundingRect.height + 4,
